...
use wasp::{runtime::{Addr, FuncInstance, GlobalInstance, HostFunc, MemInstance, Store, TableInstance, Val}, structure::{FuncType, Mem, Mutability, ValType}};
...
pub fn register_spectest(store: &mut Store) -> SpecTestExports {
    ...
    store.mems.push(MemInstance { data: vec![0u8; Mem::PAGE_SIZE], max: Some(2) });
